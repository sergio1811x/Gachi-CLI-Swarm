...
export const FLOW_CONTROL = {
  ...
  // When a viewer's in-flight + pending bytes exceed this, drop the oldest
  // buffered chunks and let the hub resync the viewer from the terminal
  // mirror snapshot instead of pausing the agent.
  DROP_HIGH_WATER: 256 * 1024,
} as const
...
interface TerminalOutputFlowOptions {
  ...
  /** Invoked once when the viewer's pending buffer overflowed and chunks were dropped. */
  onDropped: () => void
}
...
export const createTerminalOutputFlow = (
  ws: WebSocket,
  { onBackpressureChange, onDropped }: TerminalOutputFlowOptions
): TerminalOutputFlow => {
  let closed = false
  ...
  return {
    ...
    enqueue(chunk) {
      if (closed) return
      const chunkSize = byteLength(chunk)
      // Overflow guard: if the viewer is too far behind, drop the oldest
      // buffered chunks and ask the hub to resync from the mirror rather than
      // letting the pending buffer (and this viewer's memory) grow unbounded.
      if (pendingBytes + chunkSize > FLOW_CONTROL.DROP_HIGH_WATER) {
        pendingChunks = []
        pendingBytes = 0
        onDropped()
      }
      ...
      const isLowLatency =
        ...
        flushTimer === null &&
        chunkSize < FLOW_CONTROL.LOW_LATENCY_THRESHOLD_BYTES &&
