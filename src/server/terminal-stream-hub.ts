...
export const createTerminalStreamHub = (store: RuntimeStore): TerminalStreamHub => {
  ...
  const maybeResumeRun = (runId: string, state: RunState, clientId: string) => {
    ...
    if (state.backpressuredViewerIds.size === 0) store.resumeTerminalRunOutput(runId)
  }
  ...
  const cleanupViewer = (runId: string, state: RunState, clientId: string) => {
    ...
    // No one is watching this terminal — stop polling for exit. A re-attach
    // restarts the watcher. This prevents a permanent 40-poll/sec spin for
    // every terminal that was viewed while its run was still alive.
    if (state.viewers.size === 0 && state.exitInterval) {
  ...
  return {
    ...
    attachIo(runId, clientId, socket, initialSize) {
      ...
      const resyncViewer = () => {
        const controlSocket = viewer.controlSocket
        if (!controlSocket || controlSocket.readyState !== controlSocket.OPEN) return
        void state.mirror
          .getSnapshot()
          .then((snapshot) => {
            if (controlSocket.readyState === controlSocket.OPEN) {
              controlSocket.send(serializeTerminalRestore(snapshot))
            }
          })
          .catch(() => {
            if (controlSocket.readyState === controlSocket.OPEN) {
              controlSocket.send(serializeTerminalRestore(''))
            }
          })
      }
      viewer.flowState = createTerminalOutputFlow(socket, {
        onBackpressureChange(backpressured) {
          if (backpressured) {
            ...
            if (wasEmpty) store.pauseTerminalRunOutput(runId)
            return
          }
          maybeResumeRun(runId, state, clientId)
        },
        onDropped: resyncViewer,
      })
      socket.on('message', (raw, isBinary) => {
        try {
          store.writeRunInput(runId, normalizeTerminalInput(raw, isBinary))
        } catch (error) {
          // A terminal tab can keep the io socket open after its run died
          // (crash, /exit). The next keystroke must be dropped, not surfaced
          // as an uncaught PtyInactiveError that takes down the ws handler.
          if (!(error instanceof PtyInactiveError)) throw error
        }
      })
      socket.on('close', () => {
