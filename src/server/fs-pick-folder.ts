...
const finalizeWithProbe = async (path: string): Promise<PickFolderResponse> => {
  ...
  if (!probe.ok || !probe.is_dir) {
    return emptyResponse({
      error: 'Selected path is outside the browse sandbox or is not a directory.',
      path,
      probe,
    })
  }
...
const macOsPick = async (run: RunPickCommand): Promise<PickFolderResponse> => {
  const script = 'POSIX path of (choose folder with prompt "Select Gachi CLI Swarm workspace")'
...
const linuxPick = async (run: RunPickCommand): Promise<PickFolderResponse> => {
  const result = await run(
    'zenity',
    ['--file-selection', '--directory', '--title=Select Gachi CLI Swarm workspace'],
    {}
  )
  if (result.spawnError?.code === 'ENOENT') {
    return emptyResponse({
...
const windowsPick = async (run: RunPickCommand): Promise<PickFolderResponse> => {
  const script = [
    ...
    '$dialog.Description = "Select Gachi CLI Swarm workspace"',
