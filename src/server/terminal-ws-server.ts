...
export const createTerminalWebSocketServer = (
  ...
  const disposeRuntimeEventsListener = store.registerRuntimeEventsListener((workspaceId, event) => {
    tasksWss.publishEvent(workspaceId, event)
  })
