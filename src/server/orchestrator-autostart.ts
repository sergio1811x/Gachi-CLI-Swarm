...
export const autostartOrchestrator = async (
  ...
  return autostartAgent(port, workspaceId, orchestratorId, gachiPort, {
    missingConfigError:
      'No orchestrator launch config available (set GACH_ORCHESTRATOR_COMMAND or seed a role template)',
  })
}
...
export const autostartAgent = async (
  ...
  try {
    const run = await port.startAgent(workspaceId, agentId, { gachiPort })
