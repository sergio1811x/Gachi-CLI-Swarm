...
export const createTasksFileWatcher = ({
  ...
  const emitCurrentContent = async (workspaceId: string, workspacePath: string) => {
    ...
    try {
      ...
      // tasks.md is rewritten non-atomically while this watcher reads it, so
      // a transient Windows sharing violation (EBUSY/EPERM from AV or a
      // concurrent writer) is expected — log and wait for the next event
      // instead of crashing the daemon from a debounce timer.
      console.error(`[TASKS FILE] watch read failed for ${workspaceId}:`, error)
    }
  }
