...
export const createMessageLogStore = (db: Database) => {
  ...
  const insertMessage = (input: MessageLogRecord): MessageLogHandle => {
    // Belt-and-braces: a giant dispatch prompt must not blow the SQLite bind
    // limit on the message log (the PTY write carries the full payload).
    const text =
      input.text && input.text.length > 512_000
        ? `${input.text.slice(0, 512_000)}\n…[truncated ${input.text.length - 512_000} chars]`
        : input.text
    const result = db
      .prepare(
