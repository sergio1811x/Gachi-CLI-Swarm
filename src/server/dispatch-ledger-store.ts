...
export const createDispatchLedgerStore = (db: Database) => {
  ...
  /** Stamps when the payload actually settled into the worker's PTY (see the
   * stdin dispatcher's onSettled) — distinct from `submitted_at`, which only
   * means the write call was accepted, not that it reached the terminal. */
  const markDelivered = (dispatchId: string) => {
    db.prepare(
      `UPDATE dispatches
       SET delivered_at = ?
  ...
  /**
   * Cancels a dispatch in ANY non-cancelled status (including `reported`).
   *
   * Reconciliation rebuilds cards from every non-cancelled ledger row, so a
   * deleted card whose last dispatch was merely `reported` used to resurrect
   * as a zombie review entry. Deleting a card must close its whole dispatch
   * history, whatever state the rows are in.
   */
  const forceCancel = (input: { id: string; reason: string; workspaceId: string }) => {
    const existing = db
      .prepare(
        `SELECT * FROM dispatches WHERE id = ? AND workspace_id = ? AND status != 'cancelled'`
      )
      .get(input.id, input.workspaceId) as DispatchRow | undefined
    ...
    db.prepare(
      `UPDATE dispatches
       SET status = 'cancelled',
           ...
           report_text = ?
       WHERE id = ?`
    ).run(cancelledAt, input.reason, input.id)

    return {
      ...toRecord(existing),
      reportedAt: existing.reported_at ?? cancelledAt,
      reportText: input.reason,
      status: 'cancelled' as const,
    }
  }
  ...
  const getLastDispatchForWorker = (workspaceId: string, workerId: string) => {
    const row = db
      .prepare(
      ...
      .get(workspaceId, workerId) as { created_at: number; delivered_at: number | null } | undefined

    return row ? { createdAt: row.created_at, deliveredAt: row.delivered_at } : undefined
  }
  ...
  /**
   * Cancels ALL open (queued/submitted) rows for an agent. Called by the run
   * release pipeline when the owned task is requeued — the report will never
   * arrive against those rows, and a leaked `submitted` row resurrects a
   * duplicate card on every board reconcile (production: the 2026-08-30
   * restart storm spammed "ENGINE" card copies this way).
   */
  const cancelOpenForAgent = (workspaceId: string, agentId: string, reason: string) => {
    const result = db
      .prepare(
        `UPDATE dispatches
      ...
      .run(Date.now(), reason, workspaceId, agentId)
