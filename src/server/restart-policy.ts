...
import { buildAgentHandoffPrompt, loadLatestAgentSnapshot } from './agent-handoff.js'
...
export const createRestartPolicy = ({
  ...
  injectPostStartMessage({ agentId, runId, startConfig, workspace, writeToRun }) {
    ...
    // A restarted worker must not come up blind: restore the prior working
    // context (summary, changed files, remaining steps, engine, task) from the
    // handoff snapshot that the recovery watchdog persists before the stop, so
    // the replacement continues the work instead of starting from scratch.
    const handoff = loadLatestAgentSnapshot(snapshot.summary.path, agentId)
    if (handoff) {
      writeToRun(runId, buildAgentHandoffPrompt(handoff))
    }
