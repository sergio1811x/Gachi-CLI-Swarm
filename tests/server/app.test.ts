...
describe('runtime http app', () => {
  ...
  test('POST /api/workspaces creates workspace (autostart skipped)', async () => {
    ...
    const cookie = await getUiCookie(baseUrl)
    const workspacePath = mkdtempSync(join(tmpdir(), 'gachi-beta-'))
  ...
  test.skipIf(SKIP_CONPTY_WINDOWS)(
    'GET /api/ui/workspaces/:id/team returns worker team list for the UI',
    async () => {
      const { store, baseUrl } = await startServer()
  ...
  test('POST /api/ui/session/regenerate issues a new UI token and invalidates the old one', async () => {
    const { baseUrl } = await startServer()
    const uiCookie = await getUiCookie(baseUrl)

    const response = await fetch(`${baseUrl}/api/ui/session/regenerate`, {
      method: 'POST',
    ...
    const body = (await response.json()) as { ok: boolean; token: string }
    expect(body.ok).toBe(true)
    expect(response.headers.get('set-cookie')).toContain(body.token)

    const staleResponse = await fetch(`${baseUrl}/api/workspaces`, {
      headers: { cookie: uiCookie },
    })
    expect(staleResponse.status).toBe(403)

    const freshResponse = await fetch(`${baseUrl}/api/workspaces`, {
      headers: { cookie: `gachi_ui_token=${body.token}` },
    })
    expect(freshResponse.status).toBe(200)
  })

  test('POST /api/ui/session/regenerate rejects requests without a valid UI token', async () => {
    const { baseUrl } = await startServer()

    const response = await fetch(`${baseUrl}/api/ui/session/regenerate`, { method: 'POST' })
  ...
  test.skipIf(SKIP_CONPTY_WINDOWS)(
    'POST /api/workspaces/:id/workers creates a worker',
    async () => {
      const { store, baseUrl } = await startServer()
      const workspace = store.createWorkspace('/tmp/gachi-alpha', 'Alpha')
      const cookie = await getUiCookie(baseUrl)
  ...
  test.skipIf(SKIP_CONPTY_WINDOWS)(
    'DELETE /api/workspaces/:id/workers/:workerId stops active run and removes worker',
    async () => {
      const { store, baseUrl } = await startServer()
      const workspace = store.createWorkspace('/tmp/gachi-alpha', 'Alpha')
      const cookie = await getUiCookie(baseUrl)
      ...
      const startResponse = await fetch(
        `${baseUrl}/api/workspaces/${workspace.id}/agents/${worker.id}/start`,
        {
          method: 'POST',
          headers: { 'content-type': 'application/json', cookie },
          body: JSON.stringify({ gachi_port: '4010' }),
        }
      )
  ...
  test.skipIf(SKIP_CONPTY_WINDOWS)(
    'POST /api/team/send and /api/team/report update worker state',
    async () => {
      const { store, baseUrl } = await startServer()
      const workspace = store.createWorkspace('/tmp/gachi-alpha', 'Alpha')
      const cookie = await getUiCookie(baseUrl)
