...
describe('startAgent exception rollback (R1.2)', () => {
  test('marks agent stopped when launch config is missing', async () => {
    ...
    const workspace = store.createWorkspace('/tmp/gachi-rollback', 'Alpha')
    const worker = store.addWorker(workspace.id, { name: 'Alice', role: 'coder' })

    await expect(store.startAgent(workspace.id, worker.id, { gachiPort: '4010' })).rejects.toThrow(
  ...
  test('worker lands in stopped (§12) when the spawned command does not exist', async () => {
    ...
    const workspace = store.createWorkspace('/tmp/gachi-bad-spawn', 'Alpha')
    const worker = store.addWorker(workspace.id, { name: 'Alice', role: 'coder' })
  ...
  test('marks agent stopped when agentManager.startAgent throws after token issue', async () => {
    ...
    const workspace = store.createWorkspace('/tmp/gachi-rollback-spawn', 'Alpha')
    const worker = store.addWorker(workspace.id, { name: 'Alice', role: 'coder' })
