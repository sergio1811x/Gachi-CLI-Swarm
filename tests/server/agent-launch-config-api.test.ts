...
describe('agent launch config api', () => {
  test('POST /api/workspaces/:id/agents/:agentId/config stores launch config', async () => {
    const dataDir = mkdtempSync(join(tmpdir(), 'gachi-launch-config-'))
  ...
  test('POST config with only command_preset_id resolves the preset command', async () => {
    const dataDir = mkdtempSync(join(tmpdir(), 'gachi-launch-config-'))
    const workspacePath = join(dataDir, 'workspace')
    mkdirSync(workspacePath, { recursive: true })
    tempDirs.push(dataDir)
    ...
    const stored = store.peekAgentLaunchConfig(workspace.id, orchestrator.id)
    expect(stored?.command).toBe('codex')
    expect(stored?.commandPresetId).toBe('codex')
  })

  test('POST config rejects a body with no command or preset', async () => {
    const dataDir = mkdtempSync(join(tmpdir(), 'gachi-launch-config-'))
    const workspacePath = join(dataDir, 'workspace')
    mkdirSync(workspacePath, { recursive: true })
    tempDirs.push(dataDir)
