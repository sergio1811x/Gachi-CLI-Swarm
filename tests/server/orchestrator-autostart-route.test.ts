...
const makeWorkspacePath = (label: string) => {
  const dir = mkdtempSync(join(tmpdir(), `gachi-autostart-${label}-`))
  tempDirs.push(dir)
  return dir
}
...
beforeEach(() => {
  ...
  setEnv('GACH_ORCHESTRATOR_COMMAND', 'bash')
  setEnv('GACH_ORCHESTRATOR_ARGS_JSON', JSON.stringify(['-c', 'echo queen up; sleep 60']))
})

describe.skipIf(SKIP_CONPTY_WINDOWS)('POST /api/workspaces autostart_orchestrator', () => {
  test('rejects missing workspace paths before creating a workspace', async () => {
    const { store, baseUrl } = await startServer()
    const cookie = await getUiCookie(baseUrl)
    const missingPath = join(tmpdir(), `gachi-missing-${Date.now()}`)
  ...
  test('manual start during workspace autostart reuses the active orchestrator run', async () => {
    ...
    const startResponse = await fetch(
      `${baseUrl}/api/workspaces/${body.id}/agents/${body.id}:orchestrator/start`,
      {
        method: 'POST',
        headers: { 'content-type': 'application/json', cookie },
        body: JSON.stringify({ gachi_port: baseUrl.split(':').at(-1) }),
      }
    )
  ...
  test('concurrent HTTP starts for one orchestrator share a single PTY run', async () => {
    ...
    setEnv('GACH_ORCHESTRATOR_COMMAND', process.execPath)
    setEnv('GACH_ORCHESTRATOR_ARGS_JSON', JSON.stringify([scriptPath]))
  ...
  test('default Claude orchestrator launch injects bypass permission args', async () => {
    setEnv('GACH_ORCHESTRATOR_COMMAND', undefined)
    setEnv('GACH_ORCHESTRATOR_ARGS_JSON', undefined)
    ...
    const dataDir = mkdtempSync(join(tmpdir(), 'gachi-default-claude-'))
  ...
  test('UI workspace autostart uses the runtime socket port instead of client gachi_port', async () => {
    const portDir = makeWorkspacePath('runtime-port-file')
    const portFile = join(portDir, 'port.txt')
    setEnv('GACH_ORCHESTRATOR_COMMAND', process.execPath)
    setEnv(
      'GACH_ORCHESTRATOR_ARGS_JSON',
      JSON.stringify([
        '-e',
        `require('node:fs').writeFileSync(${JSON.stringify(
          portFile
        )}, readEnv('PORT') || ''); setInterval(() => {}, 1000)`,
      ])
    )
  ...
  test('command_preset_id selects the orchestrator CLI preset for autostart', async () => {
    const binDir = mkdtempSync(join(tmpdir(), 'gachi-codex-bin-'))
    const dataDir = mkdtempSync(join(tmpdir(), 'gachi-preset-codex-'))
    ...
    const codexHome = mkdtempSync(join(tmpdir(), 'gachi-codex-home-'))
  ...
  test('OpenCode orchestrator preset autostarts without Claude yolo args', async () => {
    const binDir = mkdtempSync(join(tmpdir(), 'gachi-opencode-bin-'))
    const dataDir = mkdtempSync(join(tmpdir(), 'gachi-preset-opencode-'))
    tempDirs.push(binDir)
    tempDirs.push(dataDir)
    ...
    setEnv('PATH', `${binDir}:${process.env.PATH ?? ''}`)
    const opencodeHome = mkdtempSync(join(tmpdir(), 'gachi-opencode-home-'))
    tempDirs.push(opencodeHome)
    setEnv('GACH_OPENCODE_DB_PATH', join(opencodeHome, 'opencode.db'))
  ...
  test('startup_command runs through the user shell so aliases can expand', async () => {
    const binDir = mkdtempSync(join(tmpdir(), 'gachi-custom-start-bin-'))
    const dataDir = mkdtempSync(join(tmpdir(), 'gachi-custom-start-'))
    tempDirs.push(binDir)
    tempDirs.push(dataDir)
  ...
  test('startup_command is saved when workspace creation skips autostart', async () => {
    const binDir = mkdtempSync(join(tmpdir(), 'gachi-custom-start-manual-bin-'))
    const dataDir = mkdtempSync(join(tmpdir(), 'gachi-custom-start-manual-'))
    tempDirs.push(binDir)
    tempDirs.push(dataDir)
  ...
  test('spawn failure (async exit) does NOT block workspace creation, surfaces binary name', async () => {
    setEnv('GACH_ORCHESTRATOR_COMMAND', '/definitely/not/a/real/binary')
    setEnv('GACH_ORCHESTRATOR_ARGS_JSON', '[]')
  ...
  test('async exit 127 (child dies after spawn succeeds) returns "<cmd> CLI not found in PATH"', async () => {
    ...
    setEnv('GACH_ORCHESTRATOR_COMMAND', 'bash')
    setEnv('GACH_ORCHESTRATOR_ARGS_JSON', JSON.stringify(['-c', 'exit 127']))
  ...
  test('spawn ENOENT (synchronous throw) returns "<cmd> CLI not found in PATH"', async () => {
    setEnv('GACH_ORCHESTRATOR_COMMAND', 'claude')
    setEnv('GACH_ORCHESTRATOR_ARGS_JSON', '[]')
  ...
  test('real missing command from PATH returns "<cmd> CLI not found in PATH"', async () => {
    const emptyPath = mkdtempSync(join(tmpdir(), 'gachi-empty-path-'))
  ...
  test('manual orchestrator start seeds missing launch config and uses runtime socket port', async () => {
    ...
    const response = await fetch(
      `${baseUrl}/api/workspaces/${workspace.id}/agents/${orchestratorId}/start`,
      {
        method: 'POST',
        headers: { 'content-type': 'application/json', cookie },
        body: JSON.stringify({ gachi_port: '65535' }),
      }
    )
