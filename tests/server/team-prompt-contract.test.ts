...
describe.skipIf(SKIP_CONPTY_WINDOWS)('team prompt contract', () => {
  test('team send injects sender display name, role description, and task text', async () => {
    const dataDir = mkdtempSync(join(tmpdir(), 'gachi-prompt-contract-'))
    ...
    await waitFor(() => {
      ...
      expect(output).toContain(`Your role: ${worker.description}`)
      expect(output).toContain(`Run \`team report "<result>" --dispatch ${dispatch.id}\``)
      expect(output).toContain(`dispatch_id: ${dispatch.id}`)
      expect(output).not.toContain('--success')
      expect(output).not.toContain('--failed')
      expect(output).toContain('implement login')
      // Task body is followed by a <gachi-system-reminder> tail carrying the
      // dispatch_id-bound report syntax — this is what re-anchors the worker
      // identity after an internal /compact.
      expect(output).toMatch(
        /implement login[\s\S]*<gachi-system-reminder>[\s\S]*<\/gachi-system-reminder>/
      )
  ...
  test('team send submits prompts to interactive CLI agents after bracketed paste', async () => {
    const dataDir = mkdtempSync(join(tmpdir(), 'gachi-interactive-team-send-'))
    const workspacePath = join(dataDir, 'workspace')
    ...
    await waitFor(() => {
      const run = store.getActiveRunByAgentId(workspace.id, worker.id)
      expect(run?.output).toContain('[Gachi system message: startup instructions]')
      expect(run?.output).toContain('SUBMITTED')
    }, 4000)

    await store.dispatchTaskByWorkerName(workspace.id, 'Alice', 'implement login', {
      fromAgentId: orchestrator.id,
    })
    ...
    await waitFor(() => {
      ...
      expect(run?.output).toContain(
        '\u001b[200~[Gachi system message: dispatch from @Orchestrator]'
      )
  ...
  test('team send submits to a shell-wrapped Claude worker startup command using the selected CLI driver', async () => {
    const dataDir = mkdtempSync(join(tmpdir(), 'gachi-shell-wrapped-claude-send-'))
    const workspacePath = join(dataDir, 'workspace')
    const binDir = join(dataDir, 'bin')
    mkdirSync(workspacePath, { recursive: true })
    mkdirSync(binDir, { recursive: true })
  ...
  test('team report submits to a shell-wrapped Claude startup command using the selected CLI driver', async () => {
    const dataDir = mkdtempSync(join(tmpdir(), 'gachi-shell-wrapped-claude-report-'))
    const workspacePath = join(dataDir, 'workspace')
    const binDir = join(dataDir, 'bin')
    mkdirSync(workspacePath, { recursive: true })
    mkdirSync(binDir, { recursive: true })
    ...
    await waitFor(() => {
      const run = store.getActiveRunByAgentId(workspace.id, orchestrator.id)
      expect(run?.output).toContain('\u001b[200~[Gachi system message: report from @Alice]')
