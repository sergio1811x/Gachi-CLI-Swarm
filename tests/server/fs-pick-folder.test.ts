...
beforeEach(() => {
  sandboxRoot = mkdtempSync(join(tmpdir(), 'gachi-pickfolder-root-'))
  outsideRoot = mkdtempSync(join(tmpdir(), 'gachi-pickfolder-outside-'))
...
describe('pickFolder — platform dispatch', () => {
  ...
  test('picked path outside the sandbox is rejected by probeDirectory', async () => {
    ...
    expect(result.error).toMatch(/outside the browse sandbox/)
  })
