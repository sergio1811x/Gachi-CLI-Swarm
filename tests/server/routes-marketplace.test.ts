...
beforeEach(async () => {
  vendorRoot = mkdtempSync(join(tmpdir(), 'gachi-marketplace-root-'))
  tempDirs.push(vendorRoot)
  process.env.GACH_MARKETPLACE_VENDOR_ROOT = vendorRoot
...
afterEach(async () => {
  await server.close()
  delete process.env.GACH_MARKETPLACE_VENDOR_ROOT
  delete process.env.GACH_MARKETPLACE_VENDOR_ROOT
