...
describe.skipIf(SKIP_CONPTY_WINDOWS)('runtime rehydration', () => {
  test('restores workers and pending task counts from sqlite state', () => {
    const dataDir = mkdtempSync(join(tmpdir(), 'gachi-runtime-'))
  ...
  test('restores pending task counts from dispatches instead of legacy message replay', () => {
    const dataDir = mkdtempSync(join(tmpdir(), 'gachi-runtime-dispatch-pending-'))
    tempDirs.push(dataDir)
  ...
  test('captures Claude session id into sqlite and reuses it on next start', async () => {
    const dataDir = mkdtempSync(join(tmpdir(), 'gachi-runtime-session-'))
    ...
    mkdirSync(join(claudeProjectsDir, '-tmp-gachi-alpha'), { recursive: true })
    ...
    process.env.GACH_CLAUDE_PROJECTS_DIR = claudeProjectsDir
    const sessionFile = join(
      claudeProjectsDir,
      '-tmp-gachi-alpha',
      '11111111-1111-4111-8111-111111111111.jsonl'
    )
    const workerPromptMarker = `Gachi session binding: workspace_id=${workspace.id}; agent_id=${worker.id}`
    ...
    const startPromise = secondStore.startAgent(workspace.id, worker.id, { gachiPort: '4010' })
    writeFileSync(
      sessionFile,
    ...
    await thirdStore.startAgent(workspace.id, worker.id, { gachiPort: '4010' })
