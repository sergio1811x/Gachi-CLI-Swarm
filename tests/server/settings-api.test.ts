...
describe.skipIf(SKIP_CONPTY_WINDOWS)('settings api', () => {
  ...
  test('PATCH role template preserves fields omitted from the body', async () => {
    const server = await startTestServer()
    servers.push(server)
    const cookie = await getUiCookie(server.baseUrl)
    ...
    expect(updated.default_command).toBe('claude')
    expect(updated.default_args).toEqual(['docs'])
  })
  ...
  test('PATCH command preset preserves fields omitted from the body', async () => {
    const server = await startTestServer()
    servers.push(server)
    const cookie = await getUiCookie(server.baseUrl)
    ...
    const updateResponse = await fetch(
      `${server.baseUrl}/api/settings/command-presets/${created.id}`,
      {
        method: 'PATCH',
        headers: { 'content-type': 'application/json', cookie },
        body: JSON.stringify({ display_name: 'Renamed Preset' }),
      }
    )
    expect(updateResponse.status).toBe(200)
    const updated = (await updateResponse.json()) as {
    ...
    expect(updated.display_name).toBe('Renamed Preset')
    expect(updated.command).toBe('claude')
    expect(updated.args).toEqual(['--foo'])
    expect(updated.yolo_args_template).toEqual(['--yolo'])
  })
  ...
  test('command preset responses expose executable availability', async () => {
    ...
    await expect(createResponse.json()).resolves.toEqual(
      expect.objectContaining({ available: false, command: '__GACH_missing_cli__' })
    )
    ...
    expect(presets.find((preset) => preset.command === '__GACH_missing_cli__')).toMatchObject({
      available: false,
    })
  })

  test('malformed JSON and invalid field types return 400, not 500', async () => {
    const server = await startTestServer()
    servers.push(server)
    const cookie = await getUiCookie(server.baseUrl)

    const malformedResponse = await fetch(`${server.baseUrl}/api/settings/command-presets`, {
      method: 'POST',
      headers: { 'content-type': 'application/json', cookie },
      body: '{not valid json',
    })
    ...
    const emptyResponse = await fetch(`${server.baseUrl}/api/settings/command-presets`, {
      method: 'POST',
      headers: { 'content-type': 'application/json', cookie },
      body: '',
    })
    ...
    const wrongTypeResponse = await fetch(`${server.baseUrl}/api/settings/command-presets`, {
      method: 'POST',
      headers: { 'content-type': 'application/json', cookie },
      body: JSON.stringify({ display_name: 42, command: 'claude' }),
    })
    expect(wrongTypeResponse.status).toBe(400)

    const wrongArgsResponse = await fetch(`${server.baseUrl}/api/settings/command-presets`, {
      method: 'POST',
      headers: { 'content-type': 'application/json', cookie },
      body: JSON.stringify({ display_name: 'Bad', command: 'claude', args: 'not-an-array' }),
    })
    expect(wrongArgsResponse.status).toBe(400)
  })
})
