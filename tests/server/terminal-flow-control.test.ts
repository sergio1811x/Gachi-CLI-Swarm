...
describe.skipIf(SKIP_CONPTY_WINDOWS)('terminal flow control', () => {
  test('T1 low-latency small chunk is sent without waiting for batch timer', async () => {
    ...
    const workspacePath = join(tmpdir(), `gachi-terminal-flow-direct-${Date.now()}`)
  ...
  test('T2 multiple >256B chunks batch into one websocket send', async () => {
    ...
    const workspacePath = join(tmpdir(), `gachi-terminal-flow-batch-${Date.now()}`)
    mkdirSync(workspacePath, { recursive: true })
    tempDirs.push(workspacePath)
  ...
  test('T3 unacked bytes above 100KB pauses the PTY', async () => {
    const workspacePath = join(tmpdir(), `gachi-terminal-flow-unacked-${Date.now()}`)
    mkdirSync(workspacePath, { recursive: true })
    tempDirs.push(workspacePath)
  ...
  test('T4 a slow viewer blocks resume until it disconnects', async () => {
    const workspacePath = join(tmpdir(), `gachi-terminal-flow-multi-${Date.now()}`)
    mkdirSync(workspacePath, { recursive: true })
    tempDirs.push(workspacePath)
  ...
  test('T5 bufferedAmount high water pauses and resumes the PTY', async () => {
    const workspacePath = join(tmpdir(), `gachi-terminal-flow-buffered-${Date.now()}`)
    mkdirSync(workspacePath, { recursive: true })
    tempDirs.push(workspacePath)
