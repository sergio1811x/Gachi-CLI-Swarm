...
const makeWorkspacePath = () => {
  const dir = mkdtempSync(join(tmpdir(), 'gachi-worker-autostart-'))
  tempDirs.push(dir)
  return dir
}
...
describe.skipIf(SKIP_CONPTY_WINDOWS)('POST /api/workspaces/:workspaceId/workers autostart', () => {
  ...
  test('starts a worker from a full startup command through the user shell', async () => {
    const binDir = mkdtempSync(join(tmpdir(), 'gachi-worker-custom-start-bin-'))
    const dataDir = mkdtempSync(join(tmpdir(), 'gachi-worker-custom-start-'))
  ...
  test('custom OpenCode startup command exposes the OpenCode terminal input profile', async () => {
    const binDir = mkdtempSync(join(tmpdir(), 'gachi-worker-opencode-profile-bin-'))
    const dataDir = mkdtempSync(join(tmpdir(), 'gachi-worker-opencode-profile-'))
    tempDirs.push(binDir, dataDir)
    const shellCommandFile = join(dataDir, 'shell-command.txt')
    const fakeShell = join(binDir, 'fake-zsh')
    writeFileSync(
  ...
  test('custom worker startup command reports the missing executable, not the shell', async () => {
    const dataDir = mkdtempSync(join(tmpdir(), 'gachi-worker-custom-start-missing-'))
    ...
    const cookie = await getUiCookie(server.baseUrl)
    const workspace = await createWorkspace(server.baseUrl, cookie)
    const missingCommand = 'definitely-missing-gachi-agent --serve'
    ...
    expect(body.agent_start).toMatchObject({
      error: 'definitely-missing-gachi-agent CLI not found in PATH',
      ok: false,
    })
