...
const startServer = async () => {
  const dataDir = join(tmpdir(), `gachi-tasks-api-${Date.now()}`)
...
describe('tasks api', () => {
  test('GET returns current .gachi/tasks.md content and PUT persists updates there', async () => {
    const { baseUrl, workspace } = await startServer()
    const cookie = await getUiCookie(baseUrl)
    const gachiTasksPath = join(workspace.path, '.gachi', 'tasks.md')
    ...
    expect(readFileSync(gachiTasksPath, 'utf8')).toBe('- [ ] implement login\n')
