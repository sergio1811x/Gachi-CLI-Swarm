...
interface GachiContext {
  baseUrl: string
  instance: Awaited<ReturnType<typeof runGachiCommand>>
  orchestratorId: string
  worker: { id: string; name: string }
  workspaceId: string
}

const setupGachi = async (): Promise<GachiContext> => {
  const dataDir = mkdtempSync(join(tmpdir(), 'gachi-team-authz-'))
...
describe.skipIf(IS_WINDOWS)('team API authz (R1.4)', () => {
  test('rejects spoofed orchestrator id without a valid token (401)', async () => {
    ...
    try {
      ...
      const messages = ctx.instance.store.listMessagesForRecovery(ctx.workspaceId, 0)
  ...
  test('rejects worker that invokes /api/team/send (403)', async () => {
    const ctx = await setupGachi()
    try {
      const workerToken = ctx.instance.store.peekAgentToken(ctx.worker.id)
      if (!workerToken) {
  ...
  test('rejects worker that invokes CLI team list endpoint (403)', async () => {
    ...
    try {
      ...
      const response = await fetch(`${ctx.baseUrl}/api/workspaces/${ctx.workspaceId}/team`, {
        method: 'GET',
        headers: {
          'x-gachi-agent-id': ctx.worker.id,
          'x-gachi-agent-token': workerToken,
        },
      })
      expect(response.status).toBe(403)
      expect(await response.json()).toMatchObject({ error: expect.stringContaining('coder') })
  ...
  test('orchestrator can invoke CLI team list endpoint (200)', async () => {
    const ctx = await setupGachi()
    try {
      const orchToken = ctx.instance.store.peekAgentToken(ctx.orchestratorId)
      if (!orchToken) {
      ...
      const response = await fetch(`${ctx.baseUrl}/api/workspaces/${ctx.workspaceId}/team`, {
        method: 'GET',
        headers: {
          'x-gachi-agent-id': ctx.orchestratorId,
          'x-gachi-agent-token': orchToken,
        },
      })
  ...
  test('rejects orchestrator that invokes team report (403)', async () => {
    ...
    try {
      ...
      expect(ctx.instance.store.listDispatches(ctx.workspaceId)).toEqual([])
    } finally {
      await ctx.instance.close()
    }
  })
  ...
  test('rejects token from workspace A used against workspace B (401)', async () => {
    const ctxA = await setupGachi()
    try {
      const tokenA = ctxA.instance.store.peekAgentToken(ctxA.orchestratorId)
      if (!tokenA) {
        throw new Error('Expected orchestrator token after start')
      }
      ...
      const secondWorkspacePath = mkdtempSync(join(tmpdir(), 'gachi-authz-beta-'))
  ...
  test('worker reporting with its own token succeeds (202) and decrements pending count', async () => {
    ...
    try {
      ...
      expect(ctx.instance.store.getWorker(ctx.workspaceId, ctx.worker.id).pendingTaskCount).toBe(1)
      ...
      expect(ctx.instance.store.getWorker(ctx.workspaceId, ctx.worker.id).pendingTaskCount).toBe(0)
    } finally {
      await ctx.instance.close()
    }
  })
  ...
  test('team send auto-start uses runtime socket port instead of client gachi_port', async () => {
    const ctx = await setupGachi()
    try {
      ...
      await waitFor(() => {
        ...
        expect(readFileSync(portFile, 'utf8')).toBe(String(ctx.instance.port))
      })
    } finally {
      await ctx.instance.close()
    }
