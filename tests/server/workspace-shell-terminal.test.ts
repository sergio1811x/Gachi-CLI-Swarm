...
describe.skipIf(SKIP_CONPTY_WINDOWS)('workspace shell terminal', () => {
  test('uses an unnumbered shell label after starting and replacing shells', async () => {
    const workspacePath = mkdtempSync(join(tmpdir(), 'gachi-shell-terminal-gap-'))
  ...
  test('removes a workspace shell run when the shell exits on its own', async () => {
    const workspacePath = mkdtempSync(join(tmpdir(), 'gachi-shell-terminal-exit-'))
    const binDir = mkdtempSync(join(tmpdir(), 'gachi-shell-terminal-exit-bin-'))
    tempDirs.push(workspacePath)
  ...
  test('starts one workspace shell and wires it through the terminal websocket', async () => {
    const workspacePath = mkdtempSync(join(tmpdir(), 'gachi-shell-terminal-'))
    tempDirs.push(workspacePath)
    const server = await startTestServer()
