...
describe.skipIf(SKIP_CONPTY_WINDOWS)('terminal websocket server', () => {
  test('streams PTY output over the io socket', async () => {
    const workspacePath = join(tmpdir(), `gachi-terminal-output-${Date.now()}`)
  ...
  test('forwards stdin from io socket into the PTY', async () => {
    const workspacePath = join(tmpdir(), `gachi-terminal-stdin-${Date.now()}`)
    mkdirSync(workspacePath, { recursive: true })
    tempDirs.push(workspacePath)
  ...
  test('forwards binary stdin from io socket into the PTY', async () => {
    const workspacePath = join(tmpdir(), `gachi-terminal-binary-stdin-${Date.now()}`)
    mkdirSync(workspacePath, { recursive: true })
    tempDirs.push(workspacePath)
  ...
  test('drops keystrokes for an exited run instead of crashing the ws handler', async () => {
    const workspacePath = join(tmpdir(), `gachi-terminal-dead-input-${Date.now()}`)
    mkdirSync(workspacePath, { recursive: true })
    tempDirs.push(workspacePath)
    const script = join(workspacePath, 'short-lived.js')
    writeFileSync(
      script,
      [
        "console.log('READY')",
    ...
    try {
      ...
      await waitFor(async () => {
        ...
        expect(['exited', 'error']).toContain(body.status)
      })

      // Before the typed guard this threw PtyInactiveError inside the ws
      // message handler — an uncaught exception in the daemon process.
      io.send('typed after exit\n')
      await new Promise((resolve) => setTimeout(resolve, 300))

      const health = await fetch(`${server.baseUrl}/api/runtime/runs/${run.runId}`, {
        headers: { cookie },
      })
  ...
  test('applies resize requests sent over the control socket', async () => {
    const workspacePath = join(tmpdir(), `gachi-terminal-resize-${Date.now()}`)
    mkdirSync(workspacePath, { recursive: true })
    tempDirs.push(workspacePath)
  ...
  test('control socket receives an exit event when the PTY exits', async () => {
    const workspacePath = join(tmpdir(), `gachi-terminal-exit-${Date.now()}`)
    mkdirSync(workspacePath, { recursive: true })
    tempDirs.push(workspacePath)
  ...
  test('workspace shell control socket receives an exit event when the shell exits', async () => {
    const workspacePath = join(tmpdir(), `gachi-terminal-shell-exit-${Date.now()}`)
    mkdirSync(workspacePath, { recursive: true })
    tempDirs.push(workspacePath)
