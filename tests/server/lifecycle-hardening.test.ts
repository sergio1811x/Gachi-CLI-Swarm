...
const prepareWorkspace = () => {
  const dataDir = mkdtempSync(join(tmpdir(), 'gachi-lifecycle-'))
...
describe.skipIf(SKIP_CONPTY_WINDOWS)('lifecycle hardening (R2.1 / R2.2 / R2.3) — real PTY', () => {
    ...
    try {
      const runs = await Promise.all(
        Array.from({ length: 20 }, () =>
          store.startAgent(workspace.id, worker.id, { gachiPort: '4010' })
        )
      )
  ...
  test('R2.2: double stop on a running process triggers onExit once and leaves worker stopped', async () => {
    ...
    const run = await runtime.startAgent(workspace, worker.id, { gachiPort: '4010' })
  ...
  test('R2.3: POST /api/team/send to a worker with no launch config returns 409 and records no send message', async () => {
    ...
    // Rollback: the freshly created card must not survive a failed delivery,
    // or the worker's queue accumulates zombies nobody will ever run.
  ...
  test('close waits for an in-flight agent start before tearing down live runs', async () => {
    ...
    const startPromise = store.startAgent(workspace.id, worker.id, { gachiPort: '4010' })
    await waitFor(() => {
