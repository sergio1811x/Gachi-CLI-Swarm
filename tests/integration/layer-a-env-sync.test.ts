...
describe.skipIf(IS_WINDOWS)('Layer A native resume integration', () => {
  test('successful Layer A resume does not inject fallback prompts', async () => {
    const homeDir = mkdtempSync(join(tmpdir(), 'gachi-layer-a-native-resume-home-'))
    ...
    try {
      ...
      await waitFor(async () => {
        ...
        expect(state.output).not.toContain('[Gachi system message')
      })
