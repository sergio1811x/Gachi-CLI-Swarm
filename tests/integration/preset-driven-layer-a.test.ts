...
const expectYoloMarker = join(process.cwd(), '.expect-yolo')
const geminiHome = readEnv('GEMINI_HOME') ?? join(homedir(), '.gemini')
const projectDir = join(geminiHome, 'tmp', 'gachi-test-project')
...
const db = new Database(readEnv('OPENCODE_DB_PATH'))
...
describe.skipIf(IS_WINDOWS)('preset-driven Layer A', () => {
  test('bound claude preset injects yolo args on fresh start', async () => {
    const homeDir = mkdtempSync(join(tmpdir(), 'gachi-preset-layer-a-home-'))
  ...
  test('bound claude preset binds concurrent worker session ids by worker prompt', async () => {
    const homeDir = mkdtempSync(join(tmpdir(), 'gachi-preset-layer-a-concurrent-home-'))
    const workspacePathRaw = join(homeDir, 'workspace')
    tempDirs.push(homeDir)
    mkdirSync(workspacePathRaw, { recursive: true })
    const workspacePath = realpathSync(workspacePathRaw)
  ...
  test('bound claude preset rejects another worker session id over HTTP', async () => {
    const homeDir = mkdtempSync(join(tmpdir(), 'gachi-preset-layer-a-wrong-owner-home-'))
  ...
  test.each([
    ...
    {
      env: (homeDir: string) => {
        process.env.GACH_GEMINI_HOME = join(homeDir, '.gemini')
      },
      expectedArgs: (sessionId: string) =>
    ...
    {
      env: (homeDir: string) => {
        process.env.GACH_OPENCODE_DB_PATH = join(homeDir, 'opencode.db')
      },
      expectedArgs: (sessionId: string) =>
    ...
    const homeDir = mkdtempSync(join(tmpdir(), `gachi-${input.presetId}-resume-`))
    const workspacePathRaw = join(homeDir, 'workspace')
    tempDirs.push(homeDir)
    mkdirSync(workspacePathRaw, { recursive: true })
    const workspacePath = realpathSync(workspacePathRaw)
  ...
  test('bound codex preset trusts captured session id even when the session file is gone', async () => {
    const homeDir = mkdtempSync(join(tmpdir(), 'gachi-codex-fast-resume-'))
    const workspacePathRaw = join(homeDir, 'workspace')
    tempDirs.push(homeDir)
    mkdirSync(workspacePathRaw, { recursive: true })
    const workspacePath = realpathSync(workspacePathRaw)
  ...
  test('bound codex preset still captures a session id created after CLI startup', async () => {
    const homeDir = mkdtempSync(join(tmpdir(), 'gachi-codex-delayed-resume-'))
    const workspacePathRaw = join(homeDir, 'workspace')
    tempDirs.push(homeDir)
    mkdirSync(workspacePathRaw, { recursive: true })
    const workspacePath = realpathSync(workspacePathRaw)
