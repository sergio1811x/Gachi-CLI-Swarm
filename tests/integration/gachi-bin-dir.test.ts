...
describe.skipIf(IS_WINDOWS)('gachi bin dir', () => {
  test('dummy agent can resolve team via injected GACH_BIN_DIR on PATH', async () => {
    const dataDir = mkdtempSync(join(tmpdir(), 'gachi-bin-dir-'))
  ...
  test('dummy agent can execute team list via injected GACH_BIN_DIR env', async () => {
    const dataDir = mkdtempSync(join(tmpdir(), 'gachi-bin-dir-team-list-'))
    const workspacePath = join(dataDir, 'workspace')
    mkdirSync(workspacePath, { recursive: true })
    tempDirs.push(dataDir)
