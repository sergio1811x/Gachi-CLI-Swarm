...
describeUnixOnly('gachi runtime SIGTERM shutdown', () => {
  test('SIGTERM exits runtime and cleans up active PTY child', async () => {
    const root = mkdtempSync(join(tmpdir(), 'gachi-sigterm-test-'))
    const workspacePath = join(root, 'workspace')
    tempDirs.push(root)
    mkdirSync(workspacePath, { recursive: true })
    const marker = `gachi-sigterm-marker-${crypto.randomUUID()}`
    ...
    const child = spawn(
      ...
      [
        ...
        '--input-type=module',
        '-e',
        "import { runGachiCommand } from './src/cli/gachi.ts'; await runGachiCommand(['--port','40128']);",
      ],
      {
        cwd: process.cwd(),
        env: { ...process.env, GACH_DATA_DIR: join(root, 'data') },
        stdio: 'ignore',
      }
    )
    ...
    await fetch(`${baseUrl}/api/workspaces/${workspace.id}/agents/${worker.id}/start`, {
      method: 'POST',
      headers: { 'content-type': 'application/json', cookie },
      body: JSON.stringify({ gachi_port: '40128' }),
    })
