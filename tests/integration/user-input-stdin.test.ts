...
describe.skipIf(IS_WINDOWS)('user input stdin injection', () => {
  test('user-input endpoint injects text into orchestrator PTY stdin', async () => {
    const dataDir = mkdtempSync(join(tmpdir(), 'gachi-user-input-stdin-'))
    ...
    try {
      ...
      await waitFor(async () => {
        ...
        expect(run.output).toContain('ORCH:please continue implementing login')
      })
    } finally {
