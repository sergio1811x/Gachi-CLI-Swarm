...
const openAddWorkerDialog = async (label = 'Add team member') => {
  ...
  // Under load the i18n language switch can land after the click — accept
  // either the requested localized name or its English fallback.
  try {
    return await screen.findByRole('form', { name: label }, { timeout: WORKER_FLOW_TIMEOUT_MS })
  } catch {
    return await screen.findByRole(
      'form',
      { name: /Add team member|团队成员/ },
...
describe('worker flow with real server', () => {
  // Spawns a real ConPTY worker (preset `bash -c …`) — needs an interactive
  // console; WSL-bash fallback on headless CI prints a config banner and exits.
  test.skipIf(SKIP_CONPTY_WINDOWS)(
    ...
    async () => {
      ...
      // Two close affordances exist (header X + footer Close); use the header
      // one explicitly — getAllByLabelText would throw on the pair.
      const closeButtons = within(modal).getAllByTestId('worker-modal-close')
  ...
  test.skipIf(SKIP_CONPTY_WINDOWS)(
    ...
    async () => {
      ...
      fireEvent.change(instructions, {
        target: {
          value:
            'You are a review-focused worker. Find high-risk issues first, then give minimal fix suggestions.',
        },
      })
  ...
  test('Add Worker random name follows the selected Chinese language', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Language') as HTMLSelectElement, {
      target: { value: 'zh' },
    })
    await waitFor(() => {
  ...
  test.skipIf(SKIP_CONPTY_WINDOWS)(
    'stopped worker can be started from the worker detail modal after reload',
    async () => {
