...
vi.mock('../../web/src/tasks/KanbanBoard.js', () => ({
  KanbanBoard: ({ workspaceId }: { workspaceId: string }) => (
    <div data-testid="kanban-workspace-id">{workspaceId}</div>
  ),
}))
...
// The Terminal trigger now lives in the sidebar, outside WorkspaceDetail —
// this harness stands in for it so existing "click the terminal button"
// assertions keep exercising the real shellTerminalTrigger plumbing.
const ShellTriggerHarness = (
  props: Omit<React.ComponentProps<typeof WorkspaceDetail>, 'shellTerminalTrigger'>
) => {
  ...
  return (
    <>
      ...
      />
      <WorkspaceDetail {...props} shellTerminalTrigger={trigger} />
    </>
  )
}
...
describe('WorkspaceDetail shell terminal button', () => {
  test('binds the Kanban board to the selected workspace', () => {
    renderWorkspaceDetail()

    fireEvent.click(screen.getByRole('button', { name: /Kanban board|Канбан-доска/i }))

    expect(screen.getByTestId('kanban-workspace-id')).toHaveTextContent(workspace.id)
  })
  ...
  test('panel close hides the shell panel without stopping the shell and Terminal restores it', async () => {
    ...
    window.localStorage.setItem(
      `gachi.terminal-panel.tabs.${workspace.id}`,
      JSON.stringify([`shell:${shell.run_id}`])
    )
    window.localStorage.setItem(
      `gachi.terminal-panel.active.${workspace.id}`,
      `shell:${shell.run_id}`
    )
  ...
  test('ignores a resolved shell start after switching workspaces', async () => {
    ...
    const storedTabs = window.localStorage.getItem(`gachi.terminal-panel.tabs.${workspaceTwo.id}`)
