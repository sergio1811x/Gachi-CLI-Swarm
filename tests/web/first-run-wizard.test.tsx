...
test('useFirstRunFlag returns seen=false initially and seen=true after markSeen', () => {
  ...
  expect(window.localStorage.getItem('gachi.first-run-seen')).toBe('1')
})

test('useFirstRunFlag honors existing localStorage value', () => {
  window.localStorage.setItem('gachi.first-run-seen', '1')
  const { result } = renderHook(() => useFirstRunFlag())
  expect(result.current.seen).toBe(true)
})
...
test('renders Slide 1 (Welcome) with Next button', () => {
  render(<FirstRunWizard open onClose={() => {}} onAddWorkspace={() => {}} />)
  expect(screen.getByText(/welcome to gachi/i)).toBeInTheDocument()
...
test('Skip closes the wizard from any slide', () => {
  const onClose = vi.fn()
  render(<FirstRunWizard open onClose={onClose} onAddWorkspace={() => {}} />)
  fireEvent.click(screen.getByRole('button', { name: /^skip$/i }))
  expect(onClose).toHaveBeenCalledOnce()
})

test('safety slide requires explicit acknowledgment to advance', () => {
  render(<FirstRunWizard open onClose={() => {}} onAddWorkspace={() => {}} />)
  fireEvent.click(screen.getByRole('button', { name: /^next$/i }))
  fireEvent.click(screen.getByRole('button', { name: /^next$/i }))
  expect(screen.getByText(/safety basics/i)).toBeInTheDocument()
  expect(screen.getByText(/no sandbox\. they can edit files/i)).toBeInTheDocument()
  fireEvent.click(screen.getByRole('button', { name: /^i understand$/i }))
  expect(screen.getByText(/get started/i)).toBeInTheDocument()
})

test('Slide 4 Add Workspace button fires onAddWorkspace and closes without marking seen', () => {
  const onAdd = vi.fn()
  const onClose = vi.fn()
  render(<FirstRunWizard open onClose={onClose} onAddWorkspace={onAdd} />)
  fireEvent.click(screen.getByRole('button', { name: /^next$/i }))
  fireEvent.click(screen.getByRole('button', { name: /^next$/i }))
  fireEvent.click(screen.getByRole('button', { name: /^i understand$/i }))
...
beforeEach(async () => {
  sandboxRoot = mkdtempSync(join(tmpdir(), 'gachi-first-run-wizard-'))
  mkdirSync(join(sandboxRoot, 'placeholder'), { recursive: true })
  tempDirs.push(sandboxRoot)
  process.env.GACH_FS_BROWSE_ROOT = sandboxRoot
...
afterEach(async () => {
  ...
  delete process.env.GACH_FS_BROWSE_ROOT
  delete process.env.GACH_FS_BROWSE_ROOT
...
test('wizard auto-opens when flag unset and workspaces empty', async () => {
  window.localStorage.clear()
  render(<App />)
  expect(await screen.findByRole('dialog', { name: /welcome to gachi/i })).toBeInTheDocument()
})
...
test('wizard does not open when flag is set', async () => {
  ...
  expect(screen.queryByRole('dialog', { name: /welcome to gachi/i })).toBeNull()
})

test('clicking Skip persists the flag and closes the wizard', async () => {
  window.localStorage.clear()
  render(<App />)
  await screen.findByRole('dialog', { name: /welcome to gachi/i })
  fireEvent.click(screen.getByRole('button', { name: /^skip$/i }))
  expect(window.localStorage.getItem('gachi.first-run-seen')).toBe('1')
  await waitFor(() =>
    expect(screen.queryByRole('dialog', { name: /welcome to gachi/i })).toBeNull()
