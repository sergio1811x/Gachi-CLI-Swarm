...
beforeEach(async () => {
  ...
  sandboxRoot = mkdtempSync(join(tmpdir(), 'gachi-fs-sandbox-'))
  ...
  dummyPresetId = server.store.settings.createCommandPreset({
    args: [
      '-e',
      "console.log('queen port:' + (process.env.GACH_PORT ?? process.env.GACH_PORT)); process.stdin.resume()",
    ],
...
describe('workspace flow with real server', () => {
  test('Add Workspace native-picker flow: compact confirm → create → sidebar + sub-header', async () => {
    ...
    // jsdom cannot open the native picker — take the paste-path fallback and
    // type the sandbox project path by hand.
    fireEvent.click(await screen.findByTestId('add-workspace-paste-fallback'))

    const confirm = await screen.findByTestId('confirm-workspace-dialog')
    // Paste fallback exposes the editable absolute-path field.
    fireEvent.change(within(confirm).getByTestId('confirm-workspace-paste-path'), {
      target: { value: join(sandboxRoot, 'alpha-project') },
    })
    expect(within(confirm).getByTestId('confirm-workspace-paste-path')).toHaveValue(
    ...
    // Blueprint topbar action was retired (M6-B palette); the task graph
    // drawer stays mounted but dormant (aria-hidden) until reactivated.
