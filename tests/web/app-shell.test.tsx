...
beforeEach(async () => {
  window.localStorage.removeItem?.('gachi.workspace-sidebar.width')
  window.localStorage.setItem('gachi.first-run-seen', '1')
  sandboxRoot = mkdtempSync(join(tmpdir(), 'gachi-app-shell-fs-'))
...
describe('app shell with real server', () => {
  test('renders Linear dark shell without auto-opening the folder picker on empty state', async () => {
    ...
    expect(banner.textContent ?? '').toContain('Gachi CLI Swarm')
    ...
    // jsdom cannot open the native picker — take the paste-path fallback.
  ...
  test('workspace create failure keeps dialog open and surfaces error toast', async () => {
    ...
    fireEvent.click(await screen.findByTestId('add-workspace-paste-fallback'))
    const confirm = await screen.findByTestId('confirm-workspace-dialog')
    if (!within(confirm).queryByTestId('confirm-workspace-startup-command')) {
      fireEvent.click(within(confirm).getByTestId('confirm-workspace-startup-toggle'))
    }
    // Paste mode has no probe → no suggested name; fill it explicitly.
  ...
  test('init failure surfaces error toast and swaps the workspace area for the runtime-offline page', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('toast')).toHaveTextContent(
        /could not reach Gachi CLI Swarm runtime/i
      )
    })
