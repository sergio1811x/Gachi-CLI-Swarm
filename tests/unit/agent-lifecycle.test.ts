...
describe.skipIf(SKIP_CONPTY_WINDOWS)('agent lifecycle (unit)', () => {
  test('persists launch config and records agent runs with injected gachi env', async () => {
    const dataDir = mkdtempSync(join(tmpdir(), 'gachi-agent-lifecycle-'))
    ...
    writeFileSync(
      ...
      [
        ...
        "console.log('PROJECT=' + process.env.GACH_PROJECT_ID)",
  ...
  test('can stop a running agent run', async () => {
    const dataDir = mkdtempSync(join(tmpdir(), 'gachi-agent-stop-'))
    const workspacePath = join(dataDir, 'workspace')
    mkdirSync(workspacePath, { recursive: true })
    tempDirs.push(dataDir)
