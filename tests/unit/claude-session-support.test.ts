...
const createTempRoot = () => {
  const root = join(tmpdir(), `gachi-claude-session-${crypto.randomUUID()}`)
...
describe('claude session support', () => {
  ...
  test('withPresetResumeArgs resumes an existing Claude file even when its content marker differs (S-1 lenient ownership)', () => {
    const root = createTempRoot()
    ...
    // S-1 semantics: file PRESENCE in this project dir is the ownership bound.
    // Content-marker mismatch must not veto resume — pre-marker sessions from
    // older installs would otherwise be unresumable forever.
    expect(result.resumedSessionId).toBe(sessionId)
    expect(result.args).toContain('--resume')
    expect(result.args).toContain(sessionId)
    expect(invalidSessionIds).toEqual([])
  })
