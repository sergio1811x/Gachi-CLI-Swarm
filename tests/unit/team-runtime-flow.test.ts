...
describe.skipIf(SKIP_CONPTY_WINDOWS)('team runtime flow (unit)', () => {
  test('team send injects prompt into active worker run and records a real message', async () => {
    const dataDir = mkdtempSync(join(tmpdir(), 'gachi-team-send-'))
    ...
    await waitFor(() => {
      ...
      expect(run?.output).toContain('and add tests')
    })
  ...
  test('team report injects a system message into active orchestrator run and records message', async () => {
    const dataDir = mkdtempSync(join(tmpdir(), 'gachi-team-report-'))
    const workspacePath = join(dataDir, 'workspace')
    mkdirSync(workspacePath, { recursive: true })
    tempDirs.push(dataDir)
    ...
    await waitFor(() => {
      ...
      expect(run?.output).toContain('login endpoint completed')
