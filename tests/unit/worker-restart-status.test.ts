...
describe.skipIf(SKIP_CONPTY_WINDOWS)('worker restart status (unit)', () => {
  test('starting a stopped worker restores non-stopped team status', async () => {
    const dataDir = mkdtempSync(join(tmpdir(), 'gachi-worker-restart-'))
    ...
    const firstRun = await store.startAgent(workspace.id, worker.id, { gachiPort: '4010' })
    ...
    const secondRun = await store.startAgent(workspace.id, worker.id, { gachiPort: '4010' })
