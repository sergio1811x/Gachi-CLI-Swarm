...
describe('enrichTeamList', () => {
  ...
  test('attaches commandPresetId to each worker without mutating the source array', async () => {
    ...
    const result = await enrichTeamList('ws-1', store, sourceArray)
  ...
  test('keeps commandPresetId absent when resolve returns null (does not write `undefined` into the field)', async () => {
    ...
    const store = makeStore({ launchConfigs: new Map() })
    const [result] = await enrichTeamList('ws-1', store, [worker({ id: 'w-1' })])
    expect(result).toBeDefined()
    if (!result) throw new Error('expected an enriched record')
    expect('commandPresetId' in result).toBe(false)
  })

  test('layers lastPtyLine and commandPresetId together on the same record', async () => {
  ...
  const diagnosticStore = (status: 'starting' | 'running' | 'exited') =>
    ({
      getLastPtyLineForAgent: () => 'Gemini 3.7 Flash · medium',
      peekAgentLaunchConfig: () => undefined,
      settings: { getCommandPreset: () => undefined },
      getActiveRunByAgentId: () => ({ status }),
      getLastDispatchForWorker: () => ({
        createdAt: Date.now(),
        deliveredAt: null,
      }),
    }) as unknown as TeamListEnrichmentStore

  test('does not report stuck_no_task_delivered for an idle worker (lifecycle stays consistent with status)', async () => {
    // Leftover assigned card + an ended run record must NOT surface a stuck
    // lifecycle while the worker summary is `idle` — that mismatch is what
    // confused diagnostics when a worker was freed but its card was never
    // cleared. The diagnostic only applies to a busy (`working`) worker.
    const store = diagnosticStore('exited')
    taskStore.createTask('ws-1', {
      title: 'Render A',
      assignedAgentId: 'w-1',
      status: 'assigned',
    })
    const [result] = await enrichTeamList('ws-1', store, [worker({ id: 'w-1', status: 'idle' })])
    expect(result?.lifecycleStatus).toBeUndefined()
  })

  test('reports stuck_no_task_delivered for a busy worker with an undelivered task', async () => {
    const store = diagnosticStore('running')
    taskStore.createTask('ws-1', {
      title: 'Render A',
      assignedAgentId: 'w-1',
      status: 'running',
    })
    const [result] = await enrichTeamList('ws-1', store, [worker({ id: 'w-1', status: 'working' })])
    expect(result?.lifecycleStatus).toBe('stuck_no_task_delivered')
  })

  test('exposes review work as waiting_decision without overriding a stopped worker', async () => {
    const store = makeStore({ launchConfigs: new Map() })
    taskStore.createTask('ws-1', {
      title: 'Review the implementation',
      assignedAgentId: 'waiting-worker',
      status: 'review',
    })
    taskStore.createTask('ws-1', {
      title: 'Review after process exit',
      assignedAgentId: 'stopped-worker',
      status: 'review',
    })

    const results = await enrichTeamList('ws-1', store, [
      worker({ id: 'waiting-worker', status: 'idle' }),
      worker({ id: 'stopped-worker', status: 'stopped' }),
    ])

    expect(results.map((item) => item.status)).toEqual(['waiting_decision', 'stopped'])
  })

  test('attaches rssMb from the memory-watchdog telemetry when present', async () => {
    // The watchdog cache is the only source; workers without a fresh sample
    // must keep the key absent (exactOptionalPropertyTypes — same rule as
    // commandPresetId above).
    const store = {
      ...makeStore({ launchConfigs: new Map() }),
      getWorkerRssMb: (workspaceId: string, agentId: string) =>
        workspaceId === 'ws-1' && agentId === 'w-1' ? 3456 : null,
    } as unknown as TeamListEnrichmentStore
    ...
    expect(results[0]?.rssMb).toBe(3456)
    expect('rssMb' in (results[1] ?? {})).toBe(false)
  })
})
