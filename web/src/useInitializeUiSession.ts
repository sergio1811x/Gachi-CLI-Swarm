    ...
    void initializeUiSession()
          ...
          setWorkspaces((current) => {
            ...
            if (persistedId !== nextActiveWorkspaceId) {
              saveActiveWorkspaceId(nextActiveWorkspaceId).catch((error: unknown) => {
                console.error('[gachi] swallowed:initSession.save', error)
              })
            }
            return merged
          })
      ...
      .catch((error: unknown) => {
        if (!cancelled) {
          ...
          if (onError) {
            onError('Could not reach Gachi CLI Swarm runtime. Refresh once the runtime is back up.')
          }
        }
        console.error('[gachi] swallowed:initSession.bootstrap', error)
      })
    return () => {
      cancelled = true
    }
