...
type TaskGraphDrawerProps = {
   ...
   * Omit for fixture mode where any `@token` should chip.
   */
...
export const TaskGraphDrawer = ({
  ...
  const copyTaskLine = (lineIndex: number) => {
    ...
    void navigator.clipboard?.writeText(line).catch((error: unknown) => {
      console.error('[gachi] swallowed:tasks.copyLine', error)
    })
  }
  ...
  const filePath = workspacePath ? `${workspacePath}/.gachi/tasks.md` : '.gachi/tasks.md'
