...
export interface ParseTaskMarkdownOptions {
   ...
   * behavior, useful in fixtures where worker identity is mocked.
   */
 ...
 * decompose deeply: an "implement login" parent with `- POST /login` + `- POST /logout`
 ...
 * reparse; `.gachi/tasks.md` is in practice a pure GFM checklist, so we defer
