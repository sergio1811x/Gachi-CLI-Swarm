...
import { getLastStartupCommand, setLastStartupCommand } from './workspace-startup-command.js'
...
export const ConfirmWorkspaceDialog = ({
  ...
  const [startupExpanded, setStartupExpanded] = useState(true)
  const [startupCommand, setStartupCommand] = useState(() =>
    getLastStartupCommand(commandPresetId || 'generic')
  )
  const [teamTemplates, setTeamTemplates] = useState<TeamTemplate[]>([])
  const [teamTemplateId, setTeamTemplateId] = useState('')
  const [exampleTask, setExampleTask] = useState(false)
  ...
  // Prefill the last startup command remembered for the selected orchestrator
  // CLI. Runs on mount and whenever the user switches the CLI preset.
  useEffect(() => {
    setStartupCommand(getLastStartupCommand(commandPresetId || 'generic'))
  }, [commandPresetId])
  ...
  useEffect(() => {
    ...
    listTeamTemplates()
      .then((templates) => {
        if (!cancelled) setTeamTemplates(templates)
      })
      .catch(() => {
        if (!cancelled) setTeamTemplates([])
      })
    return () => {
      cancelled = true
    }
  ...
  const handleCreate = () => {
    if (!canCreate) return
    if (startupClean) setLastStartupCommand(commandPresetId || 'generic', startupClean)
    onCreate({
          ...
          >
            ...
            <div className="flex flex-col gap-4 px-5 py-4">
              ...
              {teamTemplates.length > 0 ? (
                <label className="flex flex-col gap-2">
                  <FieldLabel>{t('teamTemplate.name')}</FieldLabel>
                  <select
                    value={teamTemplateId}
                    onChange={(event) => setTeamTemplateId(event.target.value)}
                    className="input"
                    data-testid="confirm-workspace-team-template"
                  >
                    <option value="">{t('teamTemplate.none')}</option>
                    {teamTemplates.map((template) => (
                      <option key={template.id} value={template.id}>
                        {template.name} —{' '}
                        {t('teamTemplate.workersCount', {
                          count: String(template.workers.length),
                        })}
                      </option>
                    ))}
                  </select>
              ...
              <label className="flex cursor-pointer items-start gap-2">
                <input
                  type="checkbox"
                  checked={exampleTask}
                  onChange={(event) => setExampleTask(event.target.checked)}
                  className="mt-0.5"
                  data-testid="confirm-workspace-example-task"
                />
                <span className="text-xs text-sec">{t('workspace.exampleTask')}</span>
              </label>
